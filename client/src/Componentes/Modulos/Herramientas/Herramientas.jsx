          ...
          {herramientas.map(
            (item, index) =>
              item.publico && (
                <div
                  key={index}
                  className="max-sm:-translate-y-32 max-sm:scale-90 col-span-6 max-xl:col-span-12 border-delgado rounded-md p-4 m-2"
                >
                  <div>
                    <h4 className="font-medium text-center text-2xl mb-4">
                      Herramienta {item.id} - {item.titulo}
                    </h4>
                    <table>
                      <tbody>
                        <tr>
                        ...
                        {item.procesos.map((proceso, id) => (
                          <tr className="border-delgado" key={id}>
                            ...
                            <td className="border-delgado text-center">
                              {calcularTiempo(proceso.tiempoMilis)}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                    ...
                    <div className="flex justify-center">
                      ...
                      <p className="text-justify">
                        {calcularDuracionTotal(item.procesos)}
                      </p>
                    </div>
                    ...
                    <table className="w-full">
                      <tbody>
                        <tr className="border-delgado">
                          <td className="border-delgado px-2">
                            Recomendaciones
                          </td>
